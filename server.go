...
import (
	...
	pluginapi "pluginapi/v1beta1"
)

const (
	resourceName  = "kirito/tgt_rdma"
	serverSock    = pluginapi.DevicePluginPath + "tgt_rdma.sock"
)
...
func NewDevicePlugin() (*DevicePlugin, error) {
	devicecnt, err := GetDevicesCnt()
	if err != nil {
		return nil, err
	}

	var devs = make([]*pluginapi.Device, devicecnt)
	for i := range devs {
...
func (m *DevicePlugin) Allocate(ctx context.Context, reqs *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	fmt.Printf("Allocate rdma")
	// devices, _ := GetDevicesCnt()
